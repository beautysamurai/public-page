...
Before any paid paper-analysis request, the adapter fetches the canonical PDF
(maximum 20 MiB, PDF signature checked) and counts its pages locally with pypdf.
A transport failure can
try the same version at the official `export.arxiv.org` host, with the same
strict URL/version validation. At **50 pages or fewer**, it sends those inspected
bytes inline using `file_data`. At **more than 50 pages** (including appendices),
it sends **only the arXiv abstract and locally extracted Introduction as text**;
the full PDF is never sent. Both languages explicitly disclose this limited
source and the total page count. Introduction extraction requires clear start
and next-section boundaries within the first 20 pages and 30,000 characters.
If counting or extraction fails, the review stays pending; it never silently
sends the full document or labels an abstract-only result as a full review.
PDF bytes and extracted Introduction text are not committed or published.
Logs expose only HTTP status and a
...
### Model, effort, tokens, and cost provenance

New daily/weekly/monthly JSON and Markdown records include allowlisted `usage`
metadata captured from Responses, not generated by the model: actual model
(or explicitly labelled requested model when unavailable), requested effort,
input/output/total tokens, reasoning tokens (already included in output), cached
input/cache-write tokens, source scope, page count, and a standard-rate USD
estimate when usage is complete and the model price is known. Rates are dated
2026-09-06; estimates are not invoices and exclude tax. No API keys, prompts,
...
provenance remains visible, while the period overview counts only new synthesis
calls. Past reports remain unmodified and show **not recorded**, not guessed
model names or zero tokens. Daily checkpoints retain known usage across retries;
failed calls with no usage response are explicitly unknown, never treated as free.
The page cap reduces long-PDF input cost; it is not a fixed monthly spending cap.
