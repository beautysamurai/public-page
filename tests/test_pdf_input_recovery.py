...
class PdfInputRecoveryTests(unittest.TestCase):
    ...
    def test_pdf_is_inspected_before_single_inline_request(self):
        ...
        stderr = io.StringIO()
        with mock.patch.object(p, "fetch_pdf_for_inline_input", return_value=b"%PDF-1.7\nfixture") as fetch, mock.patch.object(p, "inspect_pdf", return_value=(50, None)) as inspect, contextlib.redirect_stderr(stderr):
            result = adapter.analyze_pdf(self.candidate())
        self.assertEqual(result, analysis(importance=4))
        fetch.assert_called_once()
        inspect.assert_called_once_with(b"%PDF-1.7\nfixture")
        self.assertEqual(len(seen), 1)
        payload = seen[0]["input"][0]["content"][0]
        ...
        self.assertEqual(adapter.usage_calls[0]["pdfPages"], 50)
